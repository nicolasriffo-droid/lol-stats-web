...
# --- PANTALLA 2: PANEL DE LEAGUE OF LEGENDS ---
    ...
    with pestana1:
        ...
        respuestas = []
        # En la web usamos Sliders con soporte interactivo de ayuda (help)
        for i, preg in enumerate(preguntas, 1):
            if i == 16:
                resp = st.slider(f"{i}. {preg}", min_value=1, max_value=5, value=3, key=f"p_{i}",
                                 help="Burstear significa infligir una cantidad masiva de daño en un periodo de tiempo muy corto para eliminar a un enemigo al instante.")
            elif i == 18:
                resp = st.slider(f"{i}. {preg}", min_value=1, max_value=5, value=3, key=f"p_{i}",
                                 help="Teamfights son las peleas en equipo donde se enfrentan varios o todos los integrantes de ambos bandos al mismo tiempo.")
            elif i == 19:
                resp = st.slider(f"{i}. {preg}", min_value=1, max_value=5, value=3, key=f"p_{i}",
                                 help="Splitpushear (o empuje dividido) consiste en presionar una línea en solitario para destruir torretas mientras tu equipo distrae al enemigo en otra parte del mapa.")
            else:
                resp = st.slider(f"{i}. {preg}", min_value=1, max_value=5, value=3, key=f"p_{i}")
